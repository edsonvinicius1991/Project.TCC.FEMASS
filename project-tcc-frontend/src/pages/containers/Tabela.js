...
function Tabela({ vetorContainers, handleRemove, handleEdit, handleShow, filter }) {
    ...
    useEffect(() => {
        setContainers([...vetorContainers])
        
        filter === "Expired" ? setContainers(expiredFiltered) :
        filter === "Expiring" ? setContainers(expiringFiltered) :
        filter = null

    }, [vetorContainers])

    const equipamentosFiltrados = containers.filter((obj) => {
    ...
    }, [vetorContainers, containers, sortByField, busca, buscaCategoria, sortOrder]);
    ...
    const expiredFiltered = vetorContainers.filter(expireds)
    const expiringFiltered = vetorContainers.filter(expiringLessThanNinety)

    const expirationFilter = (e) => {
        e.preventDefault();
        if (e.target.value === 'Expiring') {
            setContainers(expiringFiltered)
        } else if (e.target.value === 'Expired') {
            setContainers(expiredFiltered)
